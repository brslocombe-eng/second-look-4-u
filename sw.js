const CACHE = 'second-look-4-u-v18';
...
function setupVoiceNotes(){
  ...
  document.querySelectorAll('[data-voice]').forEach(btn => btn.onclick = () => {
    ...
    if (!SR) { status.textContent = 'Voice input is not available here. Use the microphone on the iPhone keyboard.'; return; }
    let r = results.find(z => z.id === id); if (!r) { r={id}; results.push(r); }
    const base = note.value.trim();
    const rec = new SR(); rec.lang='en-GB'; rec.interimResults=true; rec.continuous=false;
    let finished=false; const finish=()=>{if(finished)return;finished=true;btn.classList.remove('listening');};
  ...
  const inject = voiceFix + '<script src="./ux.js"></script>';
...
self.addEventListener('install', event => {
  event.waitUntil((async()=>{
    ...
    await cache.add('./');
    try{await cache.add('./ux.js');}catch(e){}
  })());
  self.skipWaiting();
});
